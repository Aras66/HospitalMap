...
public class Database extends SQLiteAssetHelper {
    ...
    public List<String> getBudynekOpis(String budynek) {
        List<String> result = new ArrayList<>();
        if (budynek != null) {
            SQLiteDatabase db = getReadableDatabase();
            SQLiteQueryBuilder qd = new SQLiteQueryBuilder();
            ...
            String tablename = "Budynki";

            qd.setTables(tablename);
            Cursor cursor = qd.query(db, sqlSelect, "Budynek LIKE ?", new String[]{"%" + budynek + "%"}, null, null, null);

            if (cursor != null) {
                if (cursor.moveToFirst()) {
                    do {
                        result.add(cursor.getString(cursor.getColumnIndex("Opisbud")));
                    } while (cursor.moveToNext());
                }
                if (cursor == null) {
                    result.add("Nazwa budynku nieznana");
                }
            }
            return result;
        }
